...
(* let calc_minimal_energy_to_bottom (energy_map : energy_map) : Minimal_energy_map.t =
 ...
 (* IN PROGRESS - FIXING FUNCTIONALLY *)
  let calc_minimal_energy_to_bottom (energy_map: energy_map) : Minimal_energy_map.t =
      ...
      if row = rows - 1 then
        ...
        let min_neighbor_energy, min_dir =
          List.fold_left
            ...
            (Float.infinity, 0)  (* Initial values *)
            neighbors
        in
    ...
    Minimal_energy_map.iteri_bottom_to_top initial_map ~f:(fun row col pair -> let updated_pair = update_pair row col pair in Array_2d.set ~arr:initial_map ~row ~col updated_pair
    ); initial_map
